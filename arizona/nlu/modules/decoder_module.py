...
class SequenceTaggerClassifier(nn.Module):
    def __init__(
        ...
        output_dim = self.attention_embedding_dim
        
        if self.use_intent_context_concat:
            output_dim = self.attention_embedding_dim
            self.linear_out = nn.Linear(2 * attention_embedding_dim, attention_embedding_dim)
        elif self.use_intent_context_attention:
            output_dim = self.attention_embedding_dim
            self.attention = Attention(attention_embedding_dim)

        self.linear_tag = nn.Linear(input_dim, self.attention_embedding_dim, bias=False)

        if self.use_intent_context_concat or self.use_intent_context_attention:
            self.linear_intent_context = nn.Linear(self.num_intent_labels, self.attention_embedding_dim, bias=False)
            self.softmax = nn.Softmax(dim=1)  # Softmax layer for intent logits

        # TODO: Output
        self.dropout = nn.Dropout(dropout)
        self.linear = nn.Linear(output_dim, num_tag_labels)
